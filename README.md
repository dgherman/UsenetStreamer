...
- **Direct Newznab mode:** `NEWZNAB_ENABLED`, `NEWZNAB_FILTER_NZB_ONLY`, `NEWZNAB_INDEXERS` (comma-separated slot IDs of the rows to query, e.g. `01,03`; empty = all — the admin dashboard picks these by indexer name), numbered `NEWZNAB_*` entries, optional `NEWZNAB_CAPS_CACHE`.
